...
CREATE TABLE IF NOT EXISTS `mydb`.`PONUDA` (
  ...
  INDEX `fk_PONUDA_STATUS_PONUDE1_idx` (`idSTATUS_PONUDE` ASC) VISIBLE,
  CONSTRAINT `fk_PONUDA_KORISNIK1`
    FOREIGN KEY (`idKORISNIK`)
    REFERENCES `mydb`.`KORISNIK` (`idKORISNIK`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_PONUDA_STATUS_PONUDE1`
    FOREIGN KEY (`idSTATUS_PONUDE`)
    REFERENCES `mydb`.`STATUS_PONUDE` (`idSTATUS_PONUDE`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`REZERVACIJA` (
  ...
  INDEX `fk_REZERVACIJA_STATUS_REZERVACIJE1_idx` (`idSTATUS_REZERVACIJE` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_REZERVACIJA_STATUS_REZERVACIJE1`
    FOREIGN KEY (`idSTATUS_REZERVACIJE`)
    REFERENCES `mydb`.`STATUS_REZERVACIJE` (`idSTATUS_REZERVACIJE`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
